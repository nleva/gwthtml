...
class HandlerEvaluator {
	...
	HandlerEvaluator(OwnerClass ownerClass, MortalLogger logger,
		...
		handlerRegistrationJClass = oracle.findType(HandlerRegistration.class
				.getName());
	...
	public void run(IndentedWriter writer, FieldManager fieldManager,
		...
		for (JMethod method : ownerClass.getUiHandlers()) {
			...
			if (parameters.length != 1) {
				logger.die(
						"Method '%s' must have a single event parameter defined.",
						boundMethod);
			}
			...
			writeHandler(writer, uiOwner, handlerVarName, handlerType,
					eventType,
					boundMethod);
			...
			for (String objectName : annotation.value()) {
				...
				logger.getTreeLogger().log(Type.INFO, "-------------");
				logger.getTreeLogger().log(Type.INFO, "eventType = "+eventType);
				logger.getTreeLogger().log(Type.INFO, "DomEvent = "+oracle.findType(DomEvent.class.getCanonicalName()));
				logger.getTreeLogger().log(Type.INFO, "gwtEvent = "+oracle.findType(GwtEvent.class.getCanonicalName()));
				logger.getTreeLogger().log(Type.INFO, "to DomEvent ?"+eventType.isAssignableTo(oracle.findType(DomEvent.class.getCanonicalName())));
				logger.getTreeLogger().log(Type.INFO, "to GwtEvent ?"+eventType.isAssignableTo(oracle.findType(GwtEvent.class.getCanonicalName())));
				if (addHandlerMethodType == null) {
					...
					if (eventType.isAssignableTo(oracle.findType(DomEvent.class.getCanonicalName()))) {
						writeAddHandler(writer, fieldManager, handlerVarName
								+ ", " + eventType.getPackage().getName() +
								"." + eventType.getName() + ".getType()",
								"addDomHandler", objectName);
						logger.getTreeLogger().log(Type.INFO, "addDomHandler");
					} else {
						writeAddHandler(writer, fieldManager, handlerVarName
								+ ", " + eventType.getPackage().getName() +
								"." + eventType.getName() + ".getType()",
						...
						logger.getTreeLogger().log(Type.INFO, "super.addHandler");
					}
				} else {
	...
	protected void writeHandler(IndentedWriter writer, String uiOwner,
			String handlerVarName, JClassType handlerType,
		...
		if (parameters.length != 1 || parameters[0].getType() != eventType) {
			logger.die("Method '%s' needs '%s' as parameter",
					methods[0].getName(),
					eventType.getName());
		}
